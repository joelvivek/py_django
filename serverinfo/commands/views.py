...
def uploadfile_list(request):
    if request.method =='POST':
        ...
        if form.is_valid():
            ...
            for each in data:
                dbfilename = each.document
            filename = request.FILES['document'].name
            #filename = filename.replace(" ", "_")
            filelocation = form.cleaned_data["filelocation"]
            transferfile(ip,filename,filelocation,dbfilename)
            data = Document.objects.get(document = dbfilename)
            data.delete()
            os.remove(str(settings.MEDIA_ROOT)+"/"+str(dbfilename))
            msg = filename+" Successfully Uploaded at "+filelocation+" on server "+ip
...
@csrf_exempt
def deleteportmonitor_list(request):
    if request.method == 'DELETE':
        iddetails = json.loads(request.body)
        tid = iddetails.get('id')
        print(tid)
        data = portmonitor.objects.filter(id=tid)
        data.delete()
        return JsonResponse({'message': 'Monitoring Stopped Successfully'}, status=200)
    else:
        return JsonResponse({'message': 'Only DELETE method is supported'}, status=404)


@csrf_exempt
def projectportmonitor_list(request,projectname):
    if request.method == 'GET':
        data = onboardserver.objects.filter(project=str(projectname))
        if not data:
            return JsonResponse({'message': 'No Servers OnBoarded on this Project: '+projectname}, status=201)
        arr = []
        ipdetails = portmonitor.objects.values()
        for each in data:
            print(str(each.ip))
            for ipd in ipdetails:
                if(str(ipd['ip']) == str(each.ip)):
                    print(str(ipd['ip']))
                    arr.append(ipd)
        print(arr)
        jsondata = {"data":arr}
...
def javainstall_list(request):
    if request.method =='POST':
        ...
        if ip == '':
            return JsonResponse({'message':'IP should not be empty'}, status=201, safe=False)
        version = ipdetails.get('version')
        if version == '':
            return JsonResponse({'message': 'Version should not be empty'}, status=201, safe=False)
        data = onboardserver.objects.filter(ip=str(ip))
        if not data:
            return JsonResponse({'message':'Please Onboard server'}, status=201, safe=False)
        for each in data:
            javaoutput = javainstall(ip,str(each.username),str(each.password),version)
            print(javaoutput)
        return JsonResponse({'message': 'Success'}, status=200)
    else:
        return JsonResponse({'message': 'Only Post method is supported'}, status=404)
...
def transferfile(ip,filename,filelocation,dbfilename):
     ...
     print(dbfilename)
     sourcepath = str(settings.MEDIA_ROOT)+"/"+str(dbfilename)
     print(sourcepath)
     destinationpath = "/tmp/"+str(filename)
...
def javainstall(ip,username,password,version):
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    try:
        ssh.connect(hostname=ip,username=username,password=password,timeout=3)
        ...
        ssh.close()
        return responsecommand
    if version == '1.8.0_261':
        filename = "jdk-8u261-linux-x64.tar.gz"
    if version == '1.8.0_251':
        filename = "jdk-8u251-linux-x64.tar.gz"
    scp = SCPClient(ssh.get_transport())
    sourcepath = "/root/javafiles/"+filename
    print(sourcepath)
    ...
    outlines=stdout.readlines()
    print(outlines)
    stdin,stdout,stderr = ssh.exec_command("sudo tar -xvzf /tmp/"+filename+"  --directory /app/")
    outlines=stderr.readlines()
    print(outlines)

    stdin,stdout,stderr = ssh.exec_command("sudo update-alternatives --install /usr/bin/java java /app/jdk"+version+"/bin/java 2")
    outlines=stdout.readlines()
    print(outlines)

    stdin,stdout,stderr = ssh.exec_command("sudo update-alternatives --set java /app/jdk"+version+"/bin/java")
    outlines=stderr.readlines()
    print(outlines)
    return True
    '''
...
def executeanycommand(ip,username,password,command):
    ...
    try:
        ...
        ssh.close()
        return responsecommand
    if ((ip == '10.144.18.102') or (ip =='10.144.18.149')):
        command = command
    else:
